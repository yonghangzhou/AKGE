...
def add_user_friend_into_graph(g,user_friend_list):
    for pair in user_friend_list:
        ...
        g.add_edge(user_x_node, user_y_node)
    return g
...
def add_artist_tag_into_graph(g,artist_tag_list):
    for pair in artist_tag_list:
        ...
        g.add_node(artist_node)
        g.add_node(tag_node)
        g.add_edge(artist_node,tag_node)
    return g
...
def add_item_item_into_graph(g,item_item_list):
    for pair in item_item_list:
        ...
        g.add_edge(item_x_node,item_y_node)
    return g
...
def add_item_side_information(Graph, side_info_file_name):
    with open(side_info_file_name,'r') as fr_auxiliary:
        for line in fr_auxiliary:
            ...
            if r == '0':
                tail_node = 't'+tail
                Graph.add_edge(movie_node, tail_node,attr='topic')

            if r == '1':
                tail_node = 'r'+tail
                Graph.add_edge(movie_node, tail_node,attr='releases')
            if r == '2':
                tail_node = 'a'+tail
                Graph.add_edge(movie_node, tail_node,attr='artist')
            if r == '3':
                tail_node = 'e'+tail
                Graph.add_edge(movie_node, tail_node,attr='engineer')
            if r == '4':
                tail_node = 'p'+tail
                Graph.add_edge(movie_node, tail_node,attr='producer')
            if r == '5':
                tail_node = 'c'+tail
                Graph.add_edge(movie_node, tail_node,attr='canonical')
            if r == '6':
                tail_node = 'o'+tail
                Graph.add_edge(movie_node, tail_node,attr='song')
            if r == '7':
                tail_node = 's'+tail
                Graph.add_edge(movie_node, tail_node,attr='single')
            if r == '8':
                tail_node = 'f'+tail
                Graph.add_edge(movie_node, tail_node,attr='featured')
    return Graph


def graph_generation(user_artists_file_name,train_file_name,item_side_information_file_name):
    ...
    g = add_item_side_information(g, item_side_information_file_name)
...
if __name__ == '__main__':
    user_artists_file_name = 'last-fm/user_movies.txt'
    train_file_name = 'last-fm/training.txt'
    #user_friend_file_name = 'ml/user_friends.txt'
    #artist_tag_file_name = 'ml/movie_genre.txt'
    #item_item_file_name = 'ml/movie_movie.txt'
    item_side_information_file_name = 'last-fm/kg_final.txt'

    all_nodes_and_ids_path = 'last-fm/all_nodes_and_ids.pickle'

    # parameters for distance matrix
    pre_embedding_path = 'last-fm/embedding_ml.vec.json'
